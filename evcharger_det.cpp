	...
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_enumeration,"vehicle_location", PADDR(CarInformation.Location),
				...
				PT_KEYWORD,"WORK",VL_WORK,
				PT_KEYWORD,"DRIVING_HOME",VL_WORK_TO_HOME,
				PT_KEYWORD,"DRIVING_WORK",VL_HOME_TO_WORK,
		...
		switch (CarInformation.Location)
		{
			case VL_HOME:
				...
				if (t1 >= CarInformation.next_state_change)	//Nope
				{
					...
					{
						//Get the variation
						temp_double = gl_random_normal(variation_trip_mean,variation_trip_std_dev);
					...
					{
						//Get the variation
						temp_double = gl_random_normal(variation_mean,variation_std_dev) + CarInformation.HomeWorkDuration;
			...
			case VL_HOME_TO_WORK:
				...
				if (t1 >= CarInformation.next_state_change)	//Nope
				{
					...
					{
						//Get the variation
						temp_double = gl_random_normal(variation_mean,variation_std_dev) + CarInformation.WorkDuration;
			...
			case VL_WORK:
				...
				if (t1 >= CarInformation.next_state_change)	//Nope
				{
					...
					{
						//Get the variation
						temp_double = gl_random_normal(variation_mean,variation_std_dev) + CarInformation.WorkHomeDuration;
			...
			case VL_WORK_TO_HOME:
				...
				if (t1 >= CarInformation.next_state_change)	//Nope
				{
					...
					{
						//Get the variation
						temp_double = gl_random_normal(variation_mean,variation_std_dev) + CarInformation.HomeDuration;
...
EXPORT int create_evcharger_det(OBJECT **obj, OBJECT *parent)
{
	...
	{
		gl_error("create evcharger_det: %s", msg);
	}
	catch (...)
	{
		gl_error("create evcharger_det: unknown exception");
	}
	return 0;	//Failure
}
...
EXPORT int init_evcharger_det(OBJECT *obj)
{
	...
	{
		gl_error("evcharger_det %s (%s:%d): %s", obj->name, obj->oclass->name, obj->id, msg);
	}
	catch (...)
	{
		gl_error("evcharger_det %s (%s:%d): unknown exception", obj->name, obj->oclass->name, obj->id);
	}
	return 0; //Failure
}
