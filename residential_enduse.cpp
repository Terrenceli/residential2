...
int residential_enduse::init(OBJECT *parent)
{
	...
	if (load.shape!=NULL) {
		if (load.shape->schedule==NULL)
		{
			...
			/* TROUBLESHOOT
				Many residential_enduse objects require a schedule that defines how
				the load behaves.  Omitting this schedule may effectively shut the enduse
				load off if you are intending to use loadshapes.  If you are using physical
				models, direct demand input, or base_power in ZIPload, this message can be ignored.
			 */
		}
	}
