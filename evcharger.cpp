...
double evcharger::update_state(double dt /* seconds */) 
{
		...
		case VS_WORK:
			if (gl_random_bernoulli(demand.work*dt/3600))
			{
				...
				if (charge<0.25 && vehicle_type==VT_HYBRID)
					charge = 0.25;
				if (charge<=0)
					gl_warning("%s (%s:%d) vehicle has run out of charge while away from_home", obj->name?obj->name:"anonymous",obj->oclass->name,obj->id);
				else
	...
	case VS_HOME:
		if (charge<1.0 && obj->clock>0)
		{
			...
			// compute the amount of energy added for this dt - apply charger efficiency
